"""display.py — mirrors src/display.c of C step 10.

This module owns the pygame window, the CPU-side color buffer, the window
dimensions, and the drawing helpers (grid, pixel, rect, present, clear) —
main.py keeps the game loop, exactly as in C.

New in step 10: ``draw_pixel()``, the bounds-checked single-pixel write that
every later drawing routine is conceptually built on, and ``draw_rect()`` now
goes through it in C (here: the slice clamp performs the same bounds check
for the whole rectangle at once).
...
def draw_pixel(x: int, y: int, color: int) -> None:
    """Write one pixel, ignoring coordinates outside the screen.

    Mirrors draw_pixel() in display.c — new in step 10. The C indexing
    ``color_buffer[(window_width * y) + x]`` means "skip y complete rows,
    then step x pixels into that row"; our 2-D array does the same as
    ``color_buffer[y, x]``.
    """
    assert color_buffer is not None
...
def draw_rect(x: int, y: int, width: int, height: int, color: int) -> None:
    ...
    C (step 10): a double loop over width x height calling draw_pixel(), so
    off-screen parts are now safely skipped. NumPy: one 2-D slice assignment
    with the bounds clamp applied to the whole rectangle at once — the same
    per-pixel bounds check, hoisted out of the loop (CONVENTIONS.md §5).
    """
    assert color_buffer is not None
