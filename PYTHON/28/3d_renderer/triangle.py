"""triangle.py — mirrors src/triangle.c (step 28).

Owns the ``face_t`` / ``triangle_t`` types, ``draw_filled_triangle`` (the
classic flat-top/flat-bottom rasterizer), and — moved here from display.c in
this step — ``draw_triangle``, the wireframe outline. The structs also gain
``color`` (and ``avg_depth`` on triangle_t) fields in triangle.h, although
this step's main.c does not use them yet.
...
def draw_triangle(x0: int, y0: int, x1: int, y1: int, x2: int, y2: int, color: int) -> None:
    """Draw the three edges of a triangle (wireframe outline)."""
