...
Game loop for step 48 of the 3D renderer: **keyboard-controlled FPS
camera**. The automatic camera drift is gone — arrows move the camera up and
down, a/d turn (yaw), and w/s move along the current viewing direction,
everything scaled by delta_time. The look-at target is rebuilt from the yaw
angle every frame: rotate (0,0,1) by the yaw matrix, offset by the camera
position. Culling toggles move to c (on) / x (off) since d now steers. The
scene loads efa.obj/efa.png (fallback cube per CONVENTIONS.md §8) with a
tighter frustum (znear 1.0, zfar 20.0) and no mesh self-rotation.
"""
