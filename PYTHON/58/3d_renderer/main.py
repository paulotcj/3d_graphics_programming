"""main.py — mirrors src/main.c (step 58: the airport scene).

The game loop: process_input -> update -> render, at 60 FPS. NEW in this
step, the whole per-mesh pipeline moved out of ``update`` into its own
``process_graphics_pipeline_stages(mesh)`` function (the face-normal math
moved to triangle.py's ``get_triangle_normal``), and setup() now builds a
little airport: a runway plus three airplanes (f22, efa, f117) parked on it,
rendered textured by default with a far plane pushed out to 50.

Per-frame pipeline for every mesh face (process_graphics_pipeline_stages):
...
def setup() -> None:
    ...
    load_mesh("./assets/f22.obj", "./assets/f22.png", vec3_new(1, 1, 1), vec3_new(0, -1.3, +5), vec3_new(0, -math.pi / 2, 0))
    load_mesh("./assets/efa.obj", "./assets/efa.png", vec3_new(1, 1, 1), vec3_new(-2, -1.3, +9), vec3_new(0, -math.pi / 2, 0))
    load_mesh("./assets/f117.obj", "./assets/f117.png", vec3_new(1, 1, 1), vec3_new(+2, -1.3, +9), vec3_new(0, -math.pi / 2, 0))
...
def process_graphics_pipeline_stages(mesh: mesh_t) -> None:
    global view_matrix, world_matrix
        ...
        for triangle_after_clipping in triangles_after_clipping:
            ...
            for v in range(3):
                ...
                projected_point = mat4_mul_vec4(
                    proj_matrix, triangle_after_clipping.points[v]
                )
