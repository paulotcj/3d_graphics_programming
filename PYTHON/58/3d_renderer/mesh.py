...
The renderer supports multiple meshes: a static array of up to
MAX_NUM_MESHES meshes (NEW in step 58: capacity raised from 10 to 100),
each loaded with ``load_mesh(obj, png, scale, translation, rotation)`` and
carrying its **own texture** (``mesh_t.texture``). ``get_num_meshes`` /
``get_mesh`` expose the array to main.py's update loop, and the new
``rotate_mesh_x/y/z(mesh_index, angle)`` helpers nudge one mesh's rotation
(referenced from update()'s commented-out animation lines).
...
class mesh_t:
    ...
    scale: Vec3 = field(default_factory=lambda: vec3_new(1, 1, 1))  # mesh scale in x, y, and z
    rotation: Vec3 = field(default_factory=lambda: vec3_new(0, 0, 0))  # mesh rotation in x, y, and z
    translation: Vec3 = field(default_factory=lambda: vec3_new(0, 0, 0))  # mesh translation in x, y, and z
...
def rotate_mesh_x(mesh_index: int, angle: float) -> None:
    """Add to a mesh's rotation around the x-axis (NEW in step 58)."""
    meshes[mesh_index].rotation[0] += angle


def rotate_mesh_y(mesh_index: int, angle: float) -> None:
    """Add to a mesh's rotation around the y-axis (NEW in step 58)."""
    meshes[mesh_index].rotation[1] += angle


def rotate_mesh_z(mesh_index: int, angle: float) -> None:
    """Add to a mesh's rotation around the z-axis (NEW in step 58)."""
