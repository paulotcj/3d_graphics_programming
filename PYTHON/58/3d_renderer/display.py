...
# Module-level state — mirrors the static globals at the top of display.c.
...
def init_window(fullscreen: bool = False) -> bool:
    """Initialize pygame, open the window, and allocate the two buffers.

    The C step 58 opens a borderless window at the full desktop resolution
    but sizes the render buffers at HALF the desktop resolution (NEW in this
    step — the previously commented-out resize lines are back, now dividing
    by 2), letting SDL_RenderCopy stretch the buffer over the whole screen.
    ...
    — a desktop-sized borderless window with a half-resolution buffer
    scaled up 2x.
    """
    ...
    if fullscreen:
        # Mirror the C code: query the desktop resolution, open a borderless
        # window covering it, and render into a buffer at half that size
        # (stretched 2x at present time). Guarded so SDL_VIDEODRIVER=dummy
        # (which reports no real resolution) falls back to 800x600.
        info = pygame.display.Info()
        if info.current_w > 0 and info.current_h > 0:
            window_width = info.current_w // 2
            window_height = info.current_h // 2
            window = pygame.display.set_mode((info.current_w, info.current_h), pygame.NOFRAME)
        else:
    ...
    # Allocate the color buffer and the z-buffer (C: two malloc'd arrays).
...
def set_render_method(method: int) -> None:
    """Select one of the RENDER_* modes (keys 1-6)."""
    global render_method
    render_method = method


def set_cull_method(method: int) -> None:
    """Select CULL_BACKFACE (key c) or CULL_NONE (key x)."""
    global cull_method
    cull_method = method


def should_render_wire() -> bool:
    """True when the current mode draws triangle outlines."""
...
def should_render_wire_vertex() -> bool:
    """True when the current mode draws little red squares on the vertices."""
    return render_method == RENDER_WIRE_VERTEX


def should_render_filled_triangle() -> bool:
    """True when the current mode draws flat-shaded solid triangles."""
    return render_method in (RENDER_FILL_TRIANGLE, RENDER_FILL_TRIANGLE_WIRE)


def should_render_textured_triangle() -> bool:
    """True when the current mode draws perspective-correct textured triangles."""
    return render_method in (RENDER_TEXTURED, RENDER_TEXTURED_WIRE)


def should_cull_backface() -> bool:
    """True when triangles facing away from the camera should be skipped."""
...
def render_color_buffer() -> None:
    ...
    Like the C SDL_RenderCopy with a NULL destination rect, the buffer is
    stretched to the window size when the two differ (the --fullscreen case).
    """
...
def get_zbuffer_at(x: int, y: int) -> float:
    """Scalar z-buffer read with bounds check (kept for parity with the C API).

    The hot rasterizer in triangle.py reads the z-buffer as an array slice
    instead of calling this per pixel (CONVENTIONS.md §5).
    """
    if x < 0 or x >= window_width or y < 0 or y >= window_height:
        return 1.0
    assert z_buffer is not None
    return float(z_buffer[y, x])


def update_zbuffer_at(x: int, y: int, value: float) -> None:
    """Scalar z-buffer write with bounds check (kept for parity with the C API)."""
    if x < 0 or x >= window_width or y < 0 or y >= window_height:
        return
    assert z_buffer is not None
    z_buffer[y, x] = value
...
def destroy_window() -> None:
    """Shut pygame down (C: free buffers + SDL_Destroy* + SDL_Quit)."""
    pygame.quit()
