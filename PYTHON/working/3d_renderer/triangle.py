...
Owns the ``face_t`` / ``triangle_t`` types, the face-normal helper, the
barycentric-weights math, and the three triangle drawing routines
...
def get_triangle_normal(vertices: np.ndarray) -> Vec3:
    ...
    The normal is the cross product of the two edges AB and AC; its direction
    (toward or away from the camera) is what backface culling tests.
    """
...
def draw_filled_triangle(
    ...
    if not mask.any():
        return  # fully occluded or degenerate — skip the stores
