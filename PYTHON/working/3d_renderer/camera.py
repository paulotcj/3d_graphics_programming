...
The C functions pass and return ``vec3_t`` **by value**; NumPy arrays are
references, so every getter returns a copy and every setter stores a copy —
otherwise a caller mutating a returned vector in place would silently
corrupt the camera state.
"""
...
def init_camera(position: Vec3, direction: Vec3) -> None:
    ...
    _camera.position = position.copy()
    _camera.direction = direction.copy()
...
def get_camera_position() -> Vec3:
    """Return the camera position in world space (a copy, as C returns by value)."""
    return _camera.position.copy()


def get_camera_direction() -> Vec3:
    """Return the direction the camera is currently facing (a copy)."""
    return _camera.direction.copy()


def get_camera_forward_velocity() -> Vec3:
    """Return the velocity applied when moving forward/backward (a copy)."""
    return _camera.forward_velocity.copy()
...
def update_camera_forward_velocity(forward_velocity: Vec3) -> None:
    ...
    _camera.forward_velocity = forward_velocity.copy()
