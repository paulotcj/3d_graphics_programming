"""main.py — mirrors src/main.c (step 54: texture coordinates survive clipping).

The game loop: process_input -> update -> render, at 60 FPS. NEW in this
step: **UVs travel through the clipper**. When Sutherland-Hodgman splits an
edge at a frustum plane, the same interpolation factor t that places the
new vertex also lerps the texture coordinates (see float_lerp in
clipping.py), and the triangles rebuilt from the polygon carry those
clipped UVs into the rasterizer. Until now a clipped textured triangle
showed the ORIGINAL corner UVs — visibly wrong mapping at the screen edges;
from this step the texture stays glued to the geometry.
