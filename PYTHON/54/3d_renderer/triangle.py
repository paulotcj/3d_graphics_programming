...
and the three triangle drawing routines (wireframe, flat-shaded filled,
...
faster than looping in Python. (The C helpers ``draw_triangle_pixel`` and
``draw_triangle_texel`` are folded into the vectorized rasterizers.)
...
# (vectorized as a barycentric bounding-box fill — see module docstring)
###############################################################################
#
#          (x0,y0)
#            / \
...
def draw_textured_triangle(
    ...
    vectorized barycentric bounding-box rasterizer. ``texture`` is the global
    mesh texture texels array of shape ``(texture_height, texture_width)``
    (the C code reads the dimensions from the texture.c globals; here they
    are simply the array's shape). The perspective-correct part: u/w, v/w
    and 1/w are interpolated linearly across the screen, then each pixel
    recovers u = (u/w)/(1/w), v = (v/w)/(1/w).
    """
    if texture is None:
        return
