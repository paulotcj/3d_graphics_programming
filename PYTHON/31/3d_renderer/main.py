"""Step 31 — the translation matrix (mirrors src/main.c).

The homogeneous-coordinates payoff: matrix.py gains
``mat4_make_translation``, which stores the x/y/z offsets in the matrix's
last column so that a vertex with w = 1 is *moved* by the multiplication.
The hard-coded ``transformed_vertex.z += 5`` camera push is deleted —
translation is now expressed like every other transform, as a matrix. The
scale/rotation animation is commented out (exactly as in the C); instead the
mesh slides right (+0.01 x per frame) at a fixed depth of z = 5.
"""
...
import display
from matrix import mat4_make_scale, mat4_make_translation, mat4_mul_vec4
import mesh
...
def update() -> None:
    ...
    # (commented out in the C code for this step, kept for reference)
