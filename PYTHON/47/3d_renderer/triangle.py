...
Owns the ``face_t`` / ``triangle_t`` types, the barycentric-weights math, and
the three triangle drawing routines (wireframe, z-buffered flat-shaded filled,
z-buffered perspective-correct textured).
...
4. Interpolate ``1/w`` (and ``u/w``, ``v/w`` for textures) as array math —
   this is the vectorized version of draw_triangle_pixel / draw_triangle_texel.
...
###############################################################################
# Draw a filled triangle with the z-buffer depth test
###############################################################################
#
#          (x0,y0)
#            / \
...
def draw_filled_triangle(
    ...
    barycentric bounding-box rasterizer (see module docstring). The per-pixel
    body of draw_triangle_pixel — interpolate 1/w, adjust to 1 - 1/w, depth
    test, store — is steps 4-5 below, done for all pixels at once.
    """
    ...
    # z-buffer (vectorized version of the C per-pixel comparison), then update
    # the z-buffer with the 1/w of the surviving pixels.
...
def draw_textured_triangle(
    ...
    ``texture`` is the (height, width) uint32 texel array — the Python
    equivalent of the C ``uint32_t* texture`` argument. Its shape plays the
    role of the C globals texture_width/texture_height. ``None`` (no PNG
    loaded) draws nothing instead of crashing like the C would.
    """
    if texture is None:
        return
    ...
    color_slice = display.color_buffer[y_min : y_max + 1, x_min : x_max + 1]
    color_slice[mask] = texture[tex_y, tex_x]
