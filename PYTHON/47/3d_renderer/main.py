...
Step 47 introduces DELTA TIME: instead of hardcoded per-frame increments, all
animation (mesh rotation, camera drift) is scaled by the seconds elapsed since
the previous frame, so movement speed no longer depends on the frame rate. The
step also switches the model to the F-22, moves it to z = 5, and inlines the
perspective divide in main (mat4_mul_vec4_project is gone from matrix.c).
Every frame, each face travels the pipeline:
...
def setup() -> None:
    ...
    load_obj_file_data(os.path.join(ASSETS_DIR, "f22.obj"))

    # Load the texture information from an external PNG file
    load_png_texture_data(os.path.join(ASSETS_DIR, "f22.png"))
...
def update() -> None:
    ...
    mesh.rotation[0] += 0.6 * delta_time
    ...
    camera.position[0] += 0.2 * delta_time
    camera.position[1] += 0.4 * delta_time
