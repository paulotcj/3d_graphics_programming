...
Vec4 = np.ndarray  # shape (4,): [x, y, z, w] (new in step 30)
...
###############################################################################
# Vector conversion functions (new in step 30)
###############################################################################
def vec4_from_vec3(v: Vec3) -> Vec4:
    """Promote a 3D point to homogeneous coordinates with w = 1.

    w = 1 is what allows a 4x4 matrix to *translate* the point; direction
    vectors would use w = 0 so translation cannot move them.
    """
    return np.array([v[0], v[1], v[2], 1.0], dtype=np.float64)


def vec3_from_vec4(v: Vec4) -> Vec3:
    """Drop the w component, returning a plain 3D vector (a copy)."""
