"""display.py — mirrors src/display.c (step 28).

Owns the pygame window, the CPU-side color buffer, the primitive drawing
helpers (grid, pixel, line, rect), and — NEW in this step — the render/cull
mode globals from display.h (``render_method`` / ``cull_method``).
...
# enum cull_method (display.h) — how to cull faces before projection.
CULL_NONE: int = 0
CULL_BACKFACE: int = 1

# enum render_method (display.h) — what to draw for each triangle.
RENDER_WIRE: int = 0
RENDER_WIRE_VERTEX: int = 1
RENDER_FILL_TRIANGLE: int = 2
RENDER_FILL_TRIANGLE_WIRE: int = 3

# The active modes — C declares these as globals in display.h; main.py sets
# them in setup() and process_input() via ``display.render_method = ...``.
cull_method: int = CULL_BACKFACE
render_method: int = RENDER_WIRE
...
def initialize_window(fullscreen: bool = False) -> bool:
    """Initialize pygame, open the window, and report success.
    ...
    Note: the C code allocates the color buffer in main.c's ``setup()``;
    this module only opens the window here, and ``setup()`` in main.py
    creates the buffer — same split as the C files.
    """
...
def create_color_buffer() -> None:
    """Allocate the color buffer (C: the malloc in main.c's setup())."""
    global color_buffer
