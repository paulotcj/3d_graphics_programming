"""main.py — mirrors src/main.c (step 51: clipping polygons against a plane).

The game loop: process_input -> update -> render, at 60 FPS. NEW in this
step, ``clip_polygon_against_plane`` (clipping.py) gains its real
Sutherland-Hodgman implementation: each triangle is wrapped in a polygon and
clipped against all six frustum planes. The clipped polygon is **not used
yet** — breaking it back into triangles is the next lesson (the C code
leaves a "TODO" right after clip_polygon), so the original triangle is still
the one projected and drawn.

Also faithful to this step's C code: the face loop contains the debug line
``if (i != 4) continue;`` — the cube is loaded whole, but ONLY face index 4
survives the loop, so a single triangle is on screen. The mesh does not
rotate (all rotation velocities are 0.0) and sits at z = 5.

Per-frame pipeline for the (one) face:
...
3. build a polygon from the triangle and clip it (result unused for now);
4. project the original triangle, perspective-divide, and map to screen;
...
from clipping import clip_polygon, create_polygon_from_triangle, init_frustum_planes
from display import (
    CULL_BACKFACE,
    CULL_NONE,
    FPS,
...
def update() -> None:
    ...
    # (all rotation velocities are 0.0 at this step — the cube stands still)
    mesh.rotation[0] += 0.0 * delta_time
    mesh.rotation[1] += 0.0 * delta_time
    mesh.rotation[2] += 0.0 * delta_time
    ...
    for i in range(num_faces):
        # Debug line straight from this step's C code: skip every face except
        # index 4, so a single triangle of the cube is processed and drawn.
        if i != 4:
            continue
        ...
        # (next step — for now the original triangle is still the one drawn)
        ...
        for j in range(3):
            ...
            projected_point = mat4_mul_vec4(proj_matrix, transformed_vertices[j])
