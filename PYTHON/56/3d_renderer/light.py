...
# Module-level state — mirrors the `light_t light` global in light.c.
# Step 56: the C makes the global static and adds an init/getter pair —
# nothing may touch the light except through this API.
_light = light_t(direction=vec3_new(0, 0, 1))


def init_light(direction) -> None:
    """Set the scene's light direction (C: init_light)."""
    _light.direction = direction.copy()


def get_light_direction():
    """Return the light direction (a copy, matching C's by-value return)."""
    return _light.direction.copy()


def light_apply_intensity(original_color: int, percentage_factor: float) -> int:
    """Scale the RGB channels of a 0xAARRGGBB color by ``percentage_factor``.
    ...
    any bits that spilled into the neighboring channel. The factor is
    clamped to 0..1.
    """
