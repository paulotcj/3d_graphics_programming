"""main.py — mirrors src/main.c (step 56: the global light gets encapsulated).

The twin of step 55, applied to light.c: the global becomes static, set via
init_light(direction) in setup() and read via get_light_direction() in the
shading code. The mesh also spins the other way (+0.5 rad/s on x) at z = 5.
No visual pipeline changes otherwise.
"""
...
from light import get_light_direction, init_light, light_apply_intensity
from matrix import (
    mat4_look_at,
...
def setup() -> None:
    ...
    # Initialize render mode and triangle culling method (step 55: through
    # the new setter API; the buffers are allocated by the display module)
    display.set_render_method(RENDER_WIRE)
    display.set_cull_method(CULL_BACKFACE)

    # Initialize the scene light direction (new in step 56)
    ...
    # NEW in step 54: the vertical fov (fov_y) is the classic 60 degrees, and
    ...
    aspect_y = display.get_window_height() / display.get_window_width()
    aspect_x = display.get_window_width() / display.get_window_height()
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_1:
                display.set_render_method(RENDER_WIRE_VERTEX)
            if event.key == pygame.K_2:
                display.set_render_method(RENDER_WIRE)
            if event.key == pygame.K_3:
                display.set_render_method(RENDER_FILL_TRIANGLE)
            if event.key == pygame.K_4:
                display.set_render_method(RENDER_FILL_TRIANGLE_WIRE)
            if event.key == pygame.K_5:
                display.set_render_method(RENDER_TEXTURED)
            if event.key == pygame.K_6:
                display.set_render_method(RENDER_TEXTURED_WIRE)
            if event.key == pygame.K_c:
                display.set_cull_method(CULL_BACKFACE)
            if event.key == pygame.K_x:
                display.set_cull_method(CULL_NONE)
...
def update() -> None:
    ...
    mesh.rotation[0] += 0.5 * delta_time
    ...
    for i in range(num_faces):
        ...
        for triangle_after_clipping in triangles_after_clipping:
            ...
            for j in range(3):
                ...
                # Scale into the view
                projected_point[0] *= display.get_window_width() / 2.0
                projected_point[1] *= display.get_window_height() / 2.0
...
def render() -> None:
    ...
    for triangle in triangles_to_render:
        ...
        if display.should_render_filled_triangles():
        ...
        if display.should_render_textured_triangles():
        ...
        if display.should_render_wireframe():
            draw_triangle(
        ...
        if display.should_render_wire_vertex():
