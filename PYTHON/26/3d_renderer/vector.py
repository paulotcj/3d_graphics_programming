...
def vec2_normalize(v: Vec2) -> None:
    """Scale a vector **in place** to length 1 (new in step 26).

    The C function takes a vec2_t* pointer and mutates it; a numpy array is
    likewise mutated in place here, so the call sites read the same.
    """
...
def vec3_normalize(v: Vec3) -> None:
    ...
    Normalizing keeps only the *direction*: after this, dot products compare
    angles without being skewed by how long the vectors happen to be.
    """
