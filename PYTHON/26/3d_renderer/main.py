"""Step 26 — vector normalization (mirrors src/main.c).

Same culled wireframe cube as step 25, refined: vector.py gains
``vec2_normalize``/``vec3_normalize`` (in-place, like the C pointer
versions), and the culling code now normalizes the edge vectors and the
face normal. For a pure greater-/less-than-zero test the *length* of the
normal never mattered — only its direction — but normalized vectors are
about to matter a lot (lighting compares angles via dot products), so the
habit starts here.
