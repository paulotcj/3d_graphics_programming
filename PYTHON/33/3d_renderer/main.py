...
Game loop for step 33 of the 3D renderer: the rotating cube is transformed
by a single **world matrix** built with the brand-new ``mat4_mul_mat4``,
combining scale, rotation (all three axes now animate), and translation —
"[T]*[R]*[S]*v", scale first, translate last. Faces are back-face culled,
projected with the simple fov-factor perspective divide, depth-sorted with
the painter's algorithm, and drawn as wireframe / filled triangles.
...
from vector import Vec2, Vec3, vec3_new
...
###############################################################################
def setup() -> None:
    """Initialize render modes and load the mesh (C: also mallocs the buffer)."""
...
def project(point: Vec3) -> Vec2:
    """Naive perspective projection: divide x and y by depth, scale by fov."""
...
def update() -> None:
    ...
    for mesh_face in mesh.mesh.faces:
        ...
        for j in range(3):
            ...
            projected_point = project(transformed_vertices[j][:3])
