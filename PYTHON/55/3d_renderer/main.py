"""main.py — mirrors src/main.c (step 55: display state gets encapsulated).

A pure refactor with zero visual change: every display global in the C
becomes `static`, reachable only through the new accessor functions —
get_window_width/height(), set_render_method(), set_cull_method(),
is_cull_backface(), the should_render_*() predicates, and
get/update_zbuffer_at(). The buffers are also allocated by the display
module itself now instead of by setup(). main.py switches to that API
everywhere; the pipeline itself is untouched (see step 54).
"""
