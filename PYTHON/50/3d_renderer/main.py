...
Game loop for step 50 of the 3D renderer: **polygon clipping data
structures**. After backface culling, each surviving triangle is wrapped
into a polygon_t and run through clip_polygon() against all six frustum
planes. The per-plane clip is still a TODO (as in the C), so the image is
unchanged — but the pipeline seam where clipping lives is now in place;
Sutherland-Hodgman fills it in step 51, and triangulation of the clipped
polygon follows in step 52.
"""
...
def setup() -> None:
    ...
    # Initialize frustum planes with a point and a normal (new in step 49)
...
def update() -> None:
    ...
    global world_matrix, view_matrix, delta_time
    ...
    # game objects. Clamped to 0.05 s so a stall never causes a huge jump
    ...
    mesh.rotation[0] += 0.0 * delta_time  # the camera moves now, not the mesh
    mesh.rotation[1] += 0.0 * delta_time
    mesh.rotation[2] += 0.0 * delta_time
    mesh.translation[2] = 5.0

    # Initialize the target looking at the positive z-axis, rotate it by the
    # camera yaw, and offset by the camera position (new in step 48):
    target = np.array([0.0, 0.0, 1.0])
    camera_yaw_rotation = mat4_make_rotation_y(camera.yaw)
    camera.direction = (camera_yaw_rotation @ np.append(target, 1.0))[:3]
    ...
    for mesh_face in mesh.faces:
        ...
        polygon = create_polygon_from_triangle(
            transformed_vertices[0][:3],
            transformed_vertices[1][:3],
            transformed_vertices[2][:3],
        )
        ...
        for j in range(3):
            ...
            # Perform perspective divide (the original depth landed in w)
