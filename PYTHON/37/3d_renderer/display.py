...
# enum cull_method (display.h)
CULL_NONE: int = 0
CULL_BACKFACE: int = 1

# enum render_method (display.h)
RENDER_WIRE: int = 0
RENDER_WIRE_VERTEX: int = 1
RENDER_FILL_TRIANGLE: int = 2
RENDER_FILL_TRIANGLE_WIRE: int = 3
RENDER_TEXTURED: int = 4       # new in step 36
RENDER_TEXTURED_WIRE: int = 5  # new in step 36

# Module-level state — mirrors the globals at the top of display.c.
# (main.py reads/writes render_method and cull_method directly, exactly like
# the C code does with the display.h globals.)
...
def initialize_window(fullscreen: bool = False) -> bool:
    ...
    Note: the C code mallocs the color buffer in main.c's setup(); here the
    buffer lives with the window because display.py owns all display state.
    """
