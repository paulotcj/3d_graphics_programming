"""Step 6 — dotted grid via loop stride (mirrors src/main.c).

A small but instructive optimization on step 5: instead of visiting every
pixel and testing ``x % 10 == 0 || y % 10 == 0``, the C loops now *stride*
by 10 (``y += 10``, ``x += 10``) and touch only the pixels they will write.
Visually the grid becomes individual white dots at the row/column
intersections instead of solid gray lines.
...
def draw_grid() -> None:
    """Draw a white dot every 10 pixels in both directions.

    Mirrors draw_grid() in main.c, which now strides by 10 in both loops
    (``y += 10``, ``x += 10``) writing 0xFFFFFFFF — dots at the grid
    intersections only, not full lines. Performance (CONVENTIONS.md §5):
    one strided 2-D numpy slice writes exactly those pixels in a single
    array operation.
    """
    assert color_buffer is not None
    color_buffer[::10, ::10] = 0xFFFFFFFF  # a dot where both x%10==0 and y%10==0
...
def render() -> None:
    """Draw the dotted grid, present the color buffer, then clear it.

    Mirrors render() in main.c, in the same order:
    draw_grid() -> write the grid dots into the buffer,
