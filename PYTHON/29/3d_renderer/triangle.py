"""triangle.py — mirrors src/triangle.c (step 29).

Owns the ``face_t`` / ``triangle_t`` types, ``draw_filled_triangle`` (the
classic flat-top/flat-bottom rasterizer), and ``draw_triangle``, the
wireframe outline. As of this step the ``color`` field (on both structs) and
``avg_depth`` (on triangle_t) are actually used by main.c: per-face colors
and the painter's-algorithm depth sort.
