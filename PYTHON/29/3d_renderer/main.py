"""main.py — mirrors src/main.c (step 29).

The game loop: process_input -> update -> render. NEW in this step: the
painter's algorithm — every projected triangle stores its face color and the
average z-depth of its three transformed vertices, and the triangle list is
sorted back-to-front (largest avg_depth first) before rendering so nearer
faces paint over farther ones. setup() also switches back to the hard-coded
cube (load_cube_mesh_data), whose faces are now individually colored.
