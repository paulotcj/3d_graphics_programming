...
# Step 40 flips the V coordinate of every UV (v -> 1 - v): the texture rows
# are stored top-down while V grows bottom-up, so the texture now maps upright.
...
def load_obj_file_data(filename: str) -> None:
    ...
    Mirrors the C sscanf loop. New in step 42: ``vt u v`` lines are parsed
    into a local texcoords list, and each face's UV indices (the middle
    number of ``f v/vt/vn``) look up the real per-vertex UVs — textured OBJ
    models now render with their actual mapping. Normal indices are still
    ...
    texcoords: list[tex2_t] = []  # C: tex2_t* texcoords = NULL (dynamic array)
    ...
    with open(filename, "r", encoding="utf-8") as file:
        for line in file:
            ...
            # Texture coordinate information — "vt u v" (new in step 42)
            ...
            if line.startswith("f "):
                ...
                texture_indices = [int(corner.split("/")[1]) for corner in corners]
                face = face_t(
                    a=vertex_indices[0],
                    b=vertex_indices[1],
                    c=vertex_indices[2],
                    a_uv=texcoords[texture_indices[0] - 1],  # OBJ indices are 1-based
