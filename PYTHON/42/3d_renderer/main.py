...
Game loop for step 42 of the 3D renderer: **UVs from OBJ files**. The OBJ
loader now reads "vt" texture-coordinate lines and wires each face's UV
indices to them, so any textured OBJ model carries its own mapping. setup()
loads ./assets/cube.obj from disk (exercising the parser) with cube.png as
its texture, and the rasterizer flips V (texture V grows down, OBJ V grows
up). The C also drops upng_free from free_resources (the PNG data now lives
for the program's whole life).
"""
