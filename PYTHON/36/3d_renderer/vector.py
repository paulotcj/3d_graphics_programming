...
be read side by side. ``vec2_new`` / ``vec3_new`` stand in for the C struct
literals (``(vec3_t){ x, y, z }``). These scalar helpers are kept for
readability; the per-mesh vertex transform in main.py uses one NumPy matmul
instead (see CONVENTIONS.md §5).
"""
...
def vec2_from_vec4(v: Vec4) -> Vec2:
    """Keep only x and y (a copy) — used for the projected screen points."""
