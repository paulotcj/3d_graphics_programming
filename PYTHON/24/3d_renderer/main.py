"""Step 24 — the vector math library (mirrors src/main.c).

vector.py grows from just the rotation helpers into the full toolbox the
renderer will lean on from here: length, add, sub, mul, div for vec2/vec3,
plus the 3D **dot** and **cross** products (the two workhorses behind
backface culling in step 25). setup() runs a tiny demo of the new functions,
exactly like the C code, and the mesh now spins around the x axis only, so
the upcoming culling behavior is easier to observe.
...
def setup() -> None:
    ...
    mesh.load_obj_file_data(os.path.join(os.path.dirname(os.path.abspath(__file__)), "assets", "f22.obj"))

    # Quick demo of the new vector functions (mirrors the C setup() exactly;
    # the results are unused — it is a sanity check of the new library).
    a = vec3_new(2.5, 6.4, 3.0)
    b = vec3_new(-2.2, 1.4, -1.0)
    a_length = vec3_length(a)  # noqa: F841 — mirrors the C demo variables
    b_length = vec3_length(b)  # noqa: F841
    add_ab = vec3_add(a, b)    # noqa: F841
...
def update() -> None:
    ...
    mesh.mesh.rotation[1] += 0.00  # y/z rotation paused in step 24 (easier to
    mesh.mesh.rotation[2] += 0.00  # observe the upcoming culling behavior)
...
def render() -> None:
    ...
    for triangle in triangles_to_render:
        # Draw vertex points
        draw_rect(int(triangle.points[0][0]), int(triangle.points[0][1]), 3, 3, 0xFFFFFF00)  # vertex A
        draw_rect(int(triangle.points[1][0]), int(triangle.points[1][1]), 3, 3, 0xFFFFFF00)  # vertex B
        draw_rect(int(triangle.points[2][0]), int(triangle.points[2][1]), 3, 3, 0xFFFFFF00)  # vertex C
