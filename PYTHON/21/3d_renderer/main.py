"""Step 21 — Dynamic mesh (mirrors src/main.c).

The wireframe cube spins as before, but the mesh data now lives in a dynamic
``mesh_t`` structure (growable vertex/face lists plus a per-mesh rotation)
instead of fixed-size globals — the groundwork for loading arbitrary OBJ
models in the next step. The game loop is unchanged: rotate the mesh a
little each frame, transform and perspective-project every face, then draw
each projected triangle as a green wireframe with yellow vertex dots on top
of the grid.
...
def setup() -> None:
    """Allocate the color buffer and load the cube mesh (C: setup())."""
