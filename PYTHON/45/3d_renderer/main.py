...
Step 45 renders one textured, z-buffered mesh (the course loads efa.obj with
efa.png). Every frame, each face travels the pipeline:

    model space -> world matrix -> backface culling -> perspective projection
    -> screen space -> flat-shaded / textured rasterization with a z-buffer.

This is the step where the painter's-algorithm depth sort disappears for
good: BOTH the filled and the textured paths now resolve visibility per
pixel with the z-buffer, so triangle_t no longer carries an avg_depth and
triangles are rasterized in mesh order.
...
###############################################################################
def update() -> None:
    """Animate the mesh, transform, cull, light, and project every face."""
    global world_matrix
    ...
    # Batch-transform ALL mesh vertices to world space in one matmul
    ...
    for mesh_face in mesh.faces:
        ...
        transformed_vertices = world_space_vertices[
