...
Owns the pygame window, the CPU-side color buffer, the render / cull mode
flags, and the primitive drawing helpers (grid, pixel, line, rect).
...
z_buffer: np.ndarray | None = None  # (h, w) float32 of 1/w depth values (new in step 44)
...
def initialize_window(fullscreen: bool = False) -> bool:
    """Initialize pygame and open the window (C: SDL_Init + CreateWindow + CreateRenderer).
    ...
    """
    global window, color_buffer, window_width, window_height, z_buffer
    ...
    # Allocate the color buffer (C: malloc'd uint32 array in setup()).
    color_buffer = np.zeros((window_height, window_width), dtype=np.uint32)
    z_buffer = np.full((window_height, window_width), 1.0, dtype=np.float32)  # step 44
...
def clear_z_buffer() -> None:
    """Reset every z-buffer cell to 1.0 — the farthest possible depth.

    The buffer stores *adjusted* 1/w values (1.0 - 1/w), so smaller means
    closer to the camera; 1.0 means "nothing drawn here yet". C: a full
    double loop; NumPy: one broadcast (CONVENTIONS.md §5). New in step 44.
    """
