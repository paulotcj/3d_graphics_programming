...
Game loop for step 44 of the 3D renderer: **the z-buffer**. A second
full-screen buffer stores each pixel's depth (as adjusted 1/w); a textured
pixel is only drawn when it is closer than what is already there. Depth is
now resolved *per pixel* instead of per triangle, which fixes every case the
painter's algorithm gets wrong (intersecting or overlapping faces). The
painter's sort is kept only for the filled render modes — the z-buffer
replaces it for the textured ones. The scene loads f117.obj/f117.png
(fallback cube per CONVENTIONS.md §8), rotates on x, at z = 4.
"""
...
def setup() -> None:
    ...
    texture.load_png_texture_data(
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "assets", "f117.png")
    )
    ...
    mesh.load_obj_file_data(os.path.join(os.path.dirname(__file__), "assets", "f117.obj"))
...
def update() -> None:
    ...
    mesh.mesh.rotation[0] += 0.006
    mesh.mesh.rotation[1] += 0.000
    mesh.mesh.rotation[2] += 0.000
    mesh.mesh.translation[2] = 4.0
    ...
    # Step 44: the sort is only needed for the painter's-algorithm fill
    # modes — textured modes now resolve depth per pixel in the z-buffer.
