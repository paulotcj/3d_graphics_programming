"""mesh.py — mirrors src/mesh.c (step 29).

Owns the global ``mesh`` (vertices, faces, rotation), the hard-coded cube
data (NEW in this step: each cube face carries its own ARGB color), and the
OBJ file loader. The C file's dynamic ``array_push`` arrays
...
@dataclass
class mesh_t:
    """A mesh: vertex/face lists plus rotation, scale, and translation (step 30)."""
    ...
    scale: Vec3 = field(default_factory=lambda: vec3_new(1.0, 1.0, 1.0))  # x, y, z (step 30)
    translation: Vec3 = field(default_factory=lambda: vec3_new(0, 0, 0))  # x, y, z (step 30)
...
cube_faces: list[face_t] = [
    # front
    face_t(a=1, b=2, c=3, color=0xFFFF0000),
    face_t(a=1, b=3, c=4, color=0xFFFF0000),
    # right
    face_t(a=4, b=3, c=5, color=0xFF00FF00),
    face_t(a=4, b=5, c=6, color=0xFF00FF00),
    # back
    face_t(a=6, b=5, c=7, color=0xFF0000FF),
    face_t(a=6, b=7, c=8, color=0xFF0000FF),
    # left
    face_t(a=8, b=7, c=2, color=0xFFFFFF00),
    face_t(a=8, b=2, c=1, color=0xFFFFFF00),
    # top
    face_t(a=2, b=7, c=5, color=0xFFFF00FF),
    face_t(a=2, b=5, c=3, color=0xFFFF00FF),
    # bottom
    face_t(a=6, b=8, c=1, color=0xFF00FFFF),
    face_t(a=6, b=1, c=4, color=0xFF00FFFF),
]
...
def load_cube_mesh_data() -> None:
    ...
    for cube_face in cube_faces:
        mesh.faces.append(
            face_t(a=cube_face.a, b=cube_face.b, c=cube_face.c, color=cube_face.color)
        )
