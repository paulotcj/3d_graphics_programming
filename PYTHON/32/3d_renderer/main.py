"""Step 32 — rotation matrices (mirrors src/main.c).

The last of the three basic transforms becomes a matrix: matrix.py gains
mat4_make_rotation_x/y/z (cos/sin placed per axis; note the flipped sign
pattern on Y so all three spin the same direction in a left-handed system).
update() now builds all five matrices and applies them per vertex in the
order scale -> rotate x -> rotate y -> rotate z -> translate. The animation
spins the mesh around Y at a fixed depth of z = 5.

(The C step also fixes the sign convention inside the old vec3_rotate_y
helper to match the matrix version — mirrored in vector.py.)
"""
...
def update() -> None:
    ...
    # (only the y rotation animates in this step, as in the C code)
    ...
    translation_matrix = mat4_make_translation(
        mesh.mesh.translation[0], mesh.mesh.translation[1], mesh.mesh.translation[2]
    )
    ...
    for mesh_face in mesh.mesh.faces:
        ...
        for face_vertex in face_vertices:
            ...
            # Use matrices to scale, rotate, and translate our original vertex
