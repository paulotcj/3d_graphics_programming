"""display.py — mirrors src/display.c (step 27).

Owns the pygame window, the CPU-side color buffer, and the primitive drawing
helpers (grid, pixel, line, triangle outline, rect).
...
This step has no z-buffer and no render/cull mode flags yet.
"""
...
def draw_triangle(x0: int, y0: int, x1: int, y1: int, x2: int, y2: int, color: int) -> None:
    """Draw the three edges of a triangle (wireframe)."""
