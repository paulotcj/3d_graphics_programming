"""Step 4 — full-screen window via the desktop display mode (mirrors src/main.c).

Step 3 introduced the CPU-side **color buffer** (one 0xAARRGGBB ``uint32``
per pixel, streamed to the screen each frame). Step 4 changes exactly one
thing: instead of a fixed 800x600 window, the C code asks SDL for the
current display mode (``SDL_GetCurrentDisplayMode``) and sizes its
borderless window to the full desktop resolution.

Per CONVENTIONS.md §7 the Python conversion keeps a friendlier 800x600
windowed default and offers the C behavior behind a ``--fullscreen`` flag
(desktop-resolution borderless window, sized from ``pygame.display.Info()``
— the pygame equivalent of ``SDL_GetCurrentDisplayMode``).
...
def initialize_window() -> bool:
    ...
    New in step 4: the C code calls SDL_GetCurrentDisplayMode(0, ...) and
    sets window_width/window_height to the full desktop resolution before
...
def render() -> None:
    ...
    SDL_SetRenderDrawColor(0, 0, 0, 255) + SDL_RenderClear -> black clear,
    render_color_buffer() -> show the buffer,
    clear_color_buffer(0xFFFFFF00) -> reset the buffer to yellow (ARGB),
