...
Game loop for step 38 of the 3D renderer: **barycentric texture mapping**. Every mesh
face now carries per-vertex UV coordinates (texture.py introduces tex2_t and
the 64x64 red-brick texture), two new render modes appear (key 5 textured,
key 6 textured + wire, the new default), and the projected triangles hand
their texcoords to draw_textured_triangle(). That function is still a TODO
The checkerboard placeholder is gone: every pixel of every triangle now
computes its barycentric weights (how close it sits to each vertex),
interpolates the vertex UVs with them, and samples the red-brick texture.
The cube is finally textured — though with an affine mapping that will
visibly warp under perspective; the fix (1/w interpolation) lands in
step 39.
"""
...
def setup() -> None:
    ...
    # (C: mesh_texture = (uint32_t*) REDBRICK_TEXTURE — see texture.py for
    # why the array lives in assets/redbrick.png here)
    texture.mesh_texture = texture.load_redbrick_texture()
    ...
    for triangle in triangles_to_render:
        ...
        if display.render_method in (RENDER_TEXTURED, RENDER_TEXTURED_WIRE):
            draw_textured_triangle(
                int(triangle.points[0][0]), int(triangle.points[0][1]),
                triangle.texcoords[0].u, triangle.texcoords[0].v,  # vertex A
                int(triangle.points[1][0]), int(triangle.points[1][1]),
                triangle.texcoords[1].u, triangle.texcoords[1].v,  # vertex B
                int(triangle.points[2][0]), int(triangle.points[2][1]),
