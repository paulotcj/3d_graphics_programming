...
Game loop for step 43 of the 3D renderer: **a real textured OBJ model**.
setup() loads ./assets/crab.obj with ./assets/crab.png and the default
render mode becomes textured — the payoff of the last seven steps in one
frame. (The course's crab.obj was never committed to this repo, so the
loader falls back to the built-in cube textured with crab.png; drop the
original crab.obj into assets/ to see the crab. CONVENTIONS.md §8.)
The C also re-adds upng_free to free_resources and wraps texel lookups
with %% texture size — a bounds fix this conversion already had.
"""
...
def setup() -> None:
    ...
    texture.load_png_texture_data(
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "assets", "crab.png")
    )
    ...
    mesh.load_obj_file_data(os.path.join(os.path.dirname(__file__), "assets", "crab.obj"))
