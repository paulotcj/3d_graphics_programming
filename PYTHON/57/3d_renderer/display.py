...
def init_window(fullscreen: bool = False) -> bool:
    ...
    The C step 57 opens a borderless window at the full desktop resolution
    but keeps the color buffer at 800x600 (the resize lines are commented
    out), letting SDL_RenderCopy stretch the buffer over the whole screen.
    ...
    — a desktop-sized borderless window with the 800x600 buffer scaled up.
    """
    global window, color_buffer, z_buffer
    ...
    if fullscreen:
        ...
        # borderless window covering it (the render buffer stays 800x600 and
        # is stretched at present time). Guarded so SDL_VIDEODRIVER=dummy
        # (which reports no real resolution) falls back to 800x600.
        info = pygame.display.Info()
        fullscreen_width = info.current_w if info.current_w > 0 else window_width
        fullscreen_height = info.current_h if info.current_h > 0 else window_height
        window = pygame.display.set_mode((fullscreen_width, fullscreen_height), pygame.NOFRAME)
    else:
