"""main.py — mirrors src/main.c of C step 8 (the only C file in this step).

Step 8 cleans up step 7's drawing functions: ``draw_rect`` now really fills
the rectangle with the color you pass in (step 7's version overwrote the
color with a per-column counter), and the grid dots are dimmed from white
(0xFFFFFFFF) to gray (0xFFAAAAAA). The magenta 300x150 rectangle at
(300, 200) is finally visible as intended.
...
FPS: int = 60  # CONVENTIONS.md §7 frame cap (the C step 8 loop is uncapped; see README)
...
def draw_rect(x: int, y: int, width: int, height: int, color: int) -> None:
    ...
    Step 8 removes step 7's quirk: the C function now actually uses the
    ``color`` argument, filling every pixel of the width x height region
    starting at (x, y) with it.
...
def main() -> None:
    ...
    while is_running:
        ...
        clock.tick(FPS)  # CONVENTIONS.md §7 frame cap (C step 8 has none)
