"""Step 19 — line drawing (DDA) and wireframe triangles (mirrors src/main.c).

The cube finally looks like a cube: display.py gains ``draw_line()`` (the
DDA algorithm — step along the longest axis, accumulating a fixed x/y
increment) and ``draw_triangle()`` (three lines), and ``render()`` now traces
every projected face as a green wireframe on top of the yellow vertex
markers.

(The C step also adds array.c/array.h — a hand-rolled dynamic array that is
not used until step 20. Python's list already does that job, so there is
nothing to port; see README.)
"""
