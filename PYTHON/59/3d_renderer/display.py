...
def init_window(fullscreen: bool = False) -> bool:
    ...
    The C step 59 opens a borderless window at the full desktop resolution.
...
def draw_line(x0: int, y0: int, x1: int, y1: int, color: int) -> None:
    ...
    # matching the C loop's `i <= longest_side_length`). floor(v + 0.5)
    # matches C's round() (half away from zero) for on-screen coordinates;
    # np.rint would round half-to-even and land one pixel off on exact .5s.
