"""Step 22 — OBJ file loading (mirrors src/main.c).

The spinning wireframe mesh is no longer hard-coded: setup() now calls
``load_obj_file_data("./assets/cube.obj")`` and the geometry is parsed from
a Wavefront .obj file on disk. The game loop is unchanged: rotate the mesh a
...
def setup() -> None:
    ...
    # C: load_obj_file_data("./assets/cube.obj") — the path is resolved
